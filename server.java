...
public class server { 
	static int checksum;
	final static int[] checksumArray = {0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA,
	...
	//adds a header
	public static byte[] affixFlags (byte[] b, long fileLength) {
		byte[] b2 = new byte[b.length+5];
		byte[] length = new byte[] {0,0,0,0};
		b2[0] = 0;
		length = intToByte((int)fileLength);
		int i = 1;
		for(; i<5; i++) 
			b2[i] = length[i-1];
		for(; i<5; i++)
