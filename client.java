...
public class client {
	static int checksum;
	final static int[] checksumArray = { 0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA, 0x076DC419, 0x706AF48F,
	...
	public static void main(String[] args) throws Exception {
		...
		do{
			...
			while((bytesRead=is.read(segment))!=-1) {
				//if data
				if (segment[0] == 0) {
					bytesRead -= 5;
					contents = Arrays.copyOfRange(segment, 5, (segment.length));
				...
				//if EOF flag
					...
					bytesRead = 0;
				recievedFileLength += bytesRead;
			}
			//receive checksum
			byte[] check = new byte[9];
			bytesRead=is.read(check);
			byte[] fileLength= Arrays.copyOfRange(segment, 1, 4);
			int fileCheckLength = byteArrToInt(fileLength);
			//verify checksum
			...
			os.write(ackNack);
			if(ackNack[0] == 0x80)
				System.out.println("The file has been recieved.");
			else
	...
	public static void checksum(byte[] b) {
		...
		for (int i = 0; i < b.length; i = i + 4) {
			if ((i + 3) < b.length)
				byteArray = Arrays.copyOfRange(b, i, i + 3);
			else
				byteArray = Arrays.copyOfRange(b, i, b.length);
			bytes = byteArrToInt(byteArray);
			checksum += bytes;
		}
	}
